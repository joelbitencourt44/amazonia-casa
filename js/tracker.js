document.addEventListener("DOMContentLoaded", function () {
  var trackingSection = document.getElementById("tracking");
  if (trackingSection) {
...
async function loadTrackingProducts() {
  var container = document.getElementById("trackingProductsList");
  if (!container) return;
  ...
  container.innerHTML = activeProducts
    .map(function (p) {
      var imageHTML = p.image
        ...
        : '<div style="width:100%;height:200px;background:linear-gradient(135deg, #2d5a27, #4a7c3f);display:flex;align-items:center;justify-content:center;font-size:4rem;border-radius:15px 15px 0 0;">' +
          (emoji[p.category] || "🌿") +
          "</div>";

      return (
        '<div class="product-card" onclick="showTrackingDetailById(\'' +
        p.id +
...
async function showTrackingDetailById(id) {
  var product = null;

  // Buscar do Firebase
  if (typeof db !== "undefined") {
    try {
      var doc = await db.collection("products").doc(String(id)).get();
      if (doc.exists) {
        product = { id: doc.id, ...doc.data() };
      }
    } catch (e) {
      // Fallback localStorage
      var products = JSON.parse(localStorage.getItem("amazoniaProducts")) || [];
  ...
  if (product) showTrackingDetail(product);
}

function showTrackingDetail(product) {
  var modal = document.getElementById("trackingDetailModal");
  var content = document.getElementById("trackingDetailContent");
  ...
  content.innerHTML =
    imageHTML +
    '<h2 style="color:#2d5a27;margin-bottom:1rem;">' +
    product.name +
    "</h2>" +
    '<div style="background:#f9f9f9;border-radius:10px;padding:1.5rem;">' +
    '<div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.8rem;">' +
    '<span style="font-size:1.5rem;">📍</span>' +
    '<div><strong>Origem:</strong><p style="margin:0;color:#666;">' +
    (product.origin || "Não informado") +
    "</p></div>" +
    "</div>" +
    '<div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.8rem;">' +
    '<span style="font-size:1.5rem;">👤</span>' +
    '<div><strong>Extrativista:</strong><p style="margin:0;color:#666;">' +
    (product.producer || "Não informado") +
    "</p></div>" +
    "</div>" +
    '<div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.8rem;">' +
    '<span style="font-size:1.5rem;">📅</span>' +
    '<div><strong>Data da Coleta:</strong><p style="margin:0;color:#666;">' +
    (product.date || "Não informado") +
    "</p></div>" +
    "</div>" +
    '<div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.8rem;">' +
    '<span style="font-size:1.5rem;">🌳</span>' +
    '<div><strong>Área Preservada:</strong><p style="margin:0;color:#666;">' +
    (product.area || "Não informado") +
    "</p></div>" +
    "</div>" +
    (product.benefits
      ? '<div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.8rem;">' +
        '<span style="font-size:1.5rem;">✨</span>' +
        '<div><strong>Benefícios:</strong><p style="margin:0;color:#666;">' +
        product.benefits +
        "</p></div>" +
        "</div>"
      : "") +
...
window.showTrackingDetailById = showTrackingDetailById;
